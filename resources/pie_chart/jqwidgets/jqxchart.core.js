...
(function(a) {
 ...
 a.extend(a.jqx._jqxChart.prototype, {
  ...
  _credits: function() {
   if (a.jqx.credits !== "12F129D4-0E1B-44B8-9BBB-BB4CF78CC6BA") {
    var c = this;
  ...
  _showToolTip: function(J, H, n, e, b) {
   var h = this;
   var w = h._getXAxis(n);
   ...
   var g = s.series[e];
   var F = h.enableCrosshairs;
   if (h._pointMarker) {
    J = parseInt(h._pointMarker.x + 5);
    H = parseInt(h._pointMarker.y - 5)
   } else {
    F = false
   }
   var W = F && h.showToolTips == false;
   J = a.jqx._ptrnd(J);
   H = a.jqx._ptrnd(H);
   ...
   h._ttEl.iidx = b;
   var f = 0;
   for (var S = 0; S < h.seriesGroups.length; S++) {
    for (var R = 0; R < h.seriesGroups[S].series.length; R++) {
     f++
    }
   }
   if (h.showToolTipsOnAllSeries && !h._isPieGroup(n) && f > 1) {
    h._showMultiSeriesToolTip(J, H, b);
    return
   }
   var z = h._get([g.toolTipFormatSettings, s.toolTipFormatSettings, l.toolTipFormatSettings, h.toolTipFormatSettings]);
   var E = h._get([g.toolTipFormatFunction, s.toolTipFormatFunction, l.toolTipFormatFunction, h.toolTipFormatFunction]);
   var N = h._getColors(n, e, b);
   ...
   if (a.isFunction(E)) {
    ...
    for (var k in g) {
     if (k.indexOf("dataField") == 0) {
      L[k.substring(9, k.length).toLowerCase()] = h._getDataValue(b, g[k], n);
      v++
     }
    }
    if (v == 0) {
     L = h._getDataValue(b, undefined, n)
    } else {
     if (v == 1) {
      L = L[""]
     }
    ...
    C = h._getFormattedValue(n, e, b, z, E);
    var K = this._getAxisSettings(w);
    var O = K.toolTipFormatSettings;
    var T = K.toolTipFormatFunction;
    if (!T && !O && w.type == "date") {
     T = this._getDefaultDTFormatFn(w.baseUnit || "day")
    }
    var o = h._formatValue(d, O, T, n, e, b);
    if (!h._isPieGroup(n)) {
     var M = (w.displayText || w.dataField || "");
     if (M.length > 0) {
      C = M + ": " + o + "<br>" + C
     } else {
      C = o + "<br>" + C
     ...
     o = h._formatValue(d, O, T, n, e, b);
     C = o + ": " + C
    }
   }
   rect = h.renderer.getRect();
   if (F) {
    var I = a.jqx._ptrnd(h._pointMarker.x);
    var G = a.jqx._ptrnd(h._pointMarker.y);
    var B = h.crosshairsColor || h._defaultLineColor;
    if (s.polar || s.spider) {
     var D = this._getPolarAxisCoords(n, this._plotRect);
     var c = a.jqx._ptdist(I, G, D.x, D.y);
     if (c > D.r) {
      return
     }
     var A = Math.atan2(G - D.y, I - D.x);
     var p = Math.cos(A) * D.r + D.x;
     var V = Math.sin(A) * D.r + D.y;
     if (h._ttEl.vLine) {
      ...
      h._ttEl.vLine = h.renderer.line(D.x, D.y, p, V, {
       stroke: B,
     ...
     if (h._ttEl.vLine && h._ttEl.hLine) {
      ...
      h._ttEl.vLine = h.renderer.line(I, h._plotRect.y, I, h._plotRect.y + h._plotRect.height, {
       stroke: B,
       "stroke-width": h.crosshairsLineWidth || 1,
       "stroke-dasharray": h.crosshairsDashStyle || ""
      });
      h._ttEl.hLine = h.renderer.line(h._plotRect.x, G, h._plotRect.x + h._plotRect.width, G, {
       stroke: B,
       "stroke-width": h.crosshairsLineWidth || 1,
       "stroke-dasharray": h.crosshairsDashStyle || ""
      })
   ...
   if (!W && h.showToolTips != false) {
    var P = this._get([g.toolTipClass, s.toolTipClass, h.toolTipClass, this.toThemeProperty("jqx-chart-tooltip-text", null)]);
    var u = this._get([g.toolTipBackground, s.toolTipBackground, h.toolTipBackground, "#FFFFFF"]);
    var r = this._get([g.toolTipLineColor, s.toolTipLineColor, h.toolTipLineColor, N.lineColor]);
    var Q = this._get([g.toolTipOpacity, s.toolTipOpacity, h.toolTipOpacity, 1]);
    var q = this.getItemCoord(n, e, b);
    var U = 0;
    if (h._pointMarker && h._pointMarker.element) {
     U = g.symbolSizeSelected;
     if (isNaN(U)) {
      U = g.symbolSize
     }
     if (isNaN(U) || U > 50 || U < 0) {
