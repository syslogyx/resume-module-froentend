...
(function(a) {
 ...
 a.jqx.commonRenderer = {
  pieSlicePath: function(m, l, j, u, D, E, e) {
   if (!u) {
    u = 1
   }
   var o = Math.abs(D - E);
 ...
 a.jqx.svgRenderer.prototype = {
  ...
  shape: function(c, f) {
   ...
   for (var e in f) {
    if (f[e] !== undefined && f[e].toString() === "NaN") {
     d.setAttribute(e, 0)
    } else {
 ...
 a.jqx.HTML5Renderer.prototype = {
  ...
  _renderers: {
   createGradient: function(f, e, d, c) {
    f._gradients[e] = {
     orientation: d,
     colorStops: c
    }
 ...
 a.jqx.createRenderer = function(c, e) {
  ...
  setTimeout(function(){
    $('.chartData').find('.jqx-chart-legend-text').each(function(){
 ...
 a.jqx.exportImage = function(f, p, h, t, k) {
  if (!f) {
   return false
  }
  var m = p.toLowerCase() === "pdf";
  if (m) {
   p = "jpeg"
  }
  if (h == undefined || h == "") {
  ...
  try {
   if (f) {
    ...
    if (m) {
     ...
     var s;
     try {
      var s = new pdfDataExport(a.jqx.pdfExport.orientation, "pt", a.jqx.pdfExport.paperSize)
     } catch (o) {
      var s = new window.jqxPdfDataExport(a.jqx.pdfExport.orientation, "pt", a.jqx.pdfExport.paperSize)
     }
     s.addImage(i, "JPEG", 10, 10, q, 0);
     s.save(h);
     return
    }
    i = i.replace("data:image/" + p + ";base64,", "");
    if (k) {
     a.ajax({
      dataType: "string",
      url: t,
      ...
      async: false,
      success: function(w, e, z) {
       v = true
      },
      error: function(w, e, z) {
       v = false
      }
     })
    } else {
